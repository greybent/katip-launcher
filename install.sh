#!/usr/bin/env bash
# install.sh — Build and install Katip Launcher
# Run from the directory containing this file.
set -euo pipefail

EXTENSION_UUID="katip-launcher@local"
INSTALL_DIR="$HOME/.local/share/gnome-shell/extensions/$EXTENSION_UUID"
SCHEMA_ID="org.gnome.shell.extensions.katip-launcher"

# ── Count keys in schema XML ──────────────────────────────────────────────────
count_schema_keys() {
    grep -c '<key name=' "$1/schemas/org.gnome.shell.extensions.katip-launcher.gschema.xml" 2>/dev/null || echo 0
}
...
if [ "$SCHEMA_NEEDS_LOGOUT" = true ]; then
    ...
    echo "║  2. Log back in                                                  ║"
    echo "║  3. Run: gnome-extensions enable katip-launcher@local            ║"
    echo "╚══════════════════════════════════════════════════════════════════╝"
    echo ""
else
