...
export class FilesProvider extends BaseProvider {
    ...
    _initConnection() {
        try {
            ...
            console.log('[Kapit] FilesProvider: Tracker connection established');
        } catch (e) {
    ...
    async _queryTracker(text, paths) {
        ...
        if (!this._ontology) {
            ...
            console.log('[Kapit] FilesProvider detected ontology:', this._ontology.label);
        }
    ...
    async _scanDirectory(text, rootPath) {
        return new Promise(resolve => {
            GLib.idle_add(GLib.PRIORITY_DEFAULT_IDLE, () => {
                ...
                scan(rootPath, 0);
                console.log(`[Kapit] FilesProvider scanned ${rootPath}: ${results.length} results (${visited} files visited)`);
                resolve(results);
