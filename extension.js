...
export default class KapitLauncher extends Extension {
    enable() {
    ...
    _appendClipboardEntry(text) {
        const dataDir  = GLib.build_filenamev([GLib.get_user_data_dir(), 'kapit-launcher']);
    ...
    _addIndicator() {
        ...
        Main.panel.addToStatusArea('kapit-launcher', this._indicator);
    }
    ...
    _open() {
        ...
        let focusGuard = true;
        const focusGuardId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 300, () => {
            focusGuard = false;
            return GLib.SOURCE_REMOVE;
        });
